...
|-----------|---------|--------|
| **ChatInterface.php** | Main orchestrator, message handling, response parsing | âœ… Production |
| **CodeExecutor.php** | Execute PHP code safely (WP Code or eval) | âœ… Production |
| **ContextLoader.php** | Load plugin details on-demand (lazy-load) | âœ… Production |
| **ExecutionVerifier.php** | Verify action results (did CPT get created?) | âœ… Production |
| **PhaseDetector.php** | Classify response phase (discovery/proposal/execution) | âœ… Production |
| **SystemPrompts.php** | AI behavior instructions (3 phases Ã— 3 levels) | âœ… Production |
| **SnapshotManager.php** | Save/restore deltas for rollback | âœ… Production |
| **PluginDocsRepository** (Firebase) | Store plugin function docs (lazy-loaded) | âœ… Production |
...
IF WP Code snippet available:
  â””â”€ Use it (traceable, disableable, best practice)

ELSE IF WP Code not available:
  â”œâ”€ For PHP code:
  â”‚  â””â”€ Write to: wp-content/plugins/creator/codice-custom.php
  â”‚     (append to file, timestamp each section)
  â”‚
  â”œâ”€ For CSS code:
  â”‚  â””â”€ Write to: wp-content/plugins/creator/codice-custom.css
  â”‚     (append to file, timestamp each section)
  â”‚
  â”œâ”€ For JavaScript:
  â”‚  â””â”€ Write to: wp-content/plugins/creator/codice-custom.js
  â”‚     (append to file, timestamp each section)
  â”‚
  â””â”€ For functions.php modifications:
     â””â”€ Create backup of functions.php first
     â””â”€ Append code with clear markers:
...
â”œâ”€ codice-custom.php      (PHP functions, hooks)
â”œâ”€ codice-custom.css      (CSS rules)
â”œâ”€ codice-custom.js       (JavaScript)
â””â”€ codice-manifest.json   (registry of all modifications)
  {
    "modifications": [
      {
        "id": "action_123",
...
1. RECEIVE CODE REQUEST
   â”œâ”€ Check syntax: php -l or token_get_all()
   â”œâ”€ If syntax error: Return error, don't execute
   â””â”€ Continue

2. SECURITY VALIDATION
   â”œâ”€ Scan for forbidden functions (exec, shell_exec, etc.)
   â”œâ”€ Check whitelist (wp_insert_post, acf_add_field_group, etc.)
   â”œâ”€ If forbidden found: Return error, don't execute
   â””â”€ Continue

3. CREATE DELTA SNAPSHOT
   â”œâ”€ Detect affected files/DB tables
   â”œâ”€ Take "before" snapshot
   â””â”€ Store snapshot ID for rollback

4. EXECUTE CODE
   â”œâ”€ PRIMARY: Try WP Code Snippet
   â”‚  â”œâ”€ Create snippet in database
   â”‚  â”œâ”€ Snippet marked: auto_execute = true
   â”‚  â””â”€ WP loads and runs it
   â”‚
   â”œâ”€ FALLBACK (if WP Code unavailable):
   â”‚  â”œâ”€ Detect code type (PHP/CSS/JS)
   â”‚  â”œâ”€ Write to appropriate file (codice-custom.*)
   â”‚  â””â”€ eval() not preferred, but available as last resort
   â”‚
   â””â”€ Capture output + errors

5. VERIFICATION
   â”œâ”€ Run ExecutionVerifier checks
   â”œâ”€ Example: "Does CPT 'progetti' exist?"
   â”œâ”€ If verification passes: Success
   â””â”€ If verification fails: Prepare for retry

6. RETURN RESULT
   â”œâ”€ success: true/false
   â”œâ”€ message: User-friendly description
   â”œâ”€ snapshot_id: For rollback button
   â””â”€ verification: What was checked
```
...
3. Verify it's available in repository
   ...
   â”œâ”€ If not cached: AI research + save to repository
