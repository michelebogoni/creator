 ...
 * Completely removes all plugin data for a clean reinstall.
 *
 * @package CreatorCore
 */
 ...
 *
 * Note: This will permanently delete all Creator data including
 * database tables, options, transients, files, and backups.
 */
...
$options = [
    ...
    'creator_do_activation_redirect',
    'creator_default_model',
    'creator_default_tier',
];
...
// Also clean up any transients with creator_ prefix from the database.
$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_creator_%'" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_timeout_creator_%'" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
