...
class ActionExecutor {
    ...
    /**
     * Normalize action type to handle AI-generated variations
     *
     * Maps semantic action types generated by AI to supported internal types.
     *
     * @param string $action_type Original action type.
     * @param array  $params      Action parameters.
     * @return array Normalized type and params.
     */
    private function normalize_action_type( string $action_type, array $params ): array {
        // Define mappings from AI-generated types to supported types
        $mappings = [
            // Elementor variations
            'create_elementor_page'   => [ 'type' => 'create_page', 'params' => [ 'use_elementor' => true ] ],
            'elementor_create_page'   => [ 'type' => 'create_page', 'params' => [ 'use_elementor' => true ] ],
            'new_elementor_page'      => [ 'type' => 'create_page', 'params' => [ 'use_elementor' => true ] ],
            'add_elementor_page'      => [ 'type' => 'create_page', 'params' => [ 'use_elementor' => true ] ],
            'build_elementor_page'    => [ 'type' => 'create_page', 'params' => [ 'use_elementor' => true ] ],

            // Post variations
            'new_post'                => [ 'type' => 'create_post', 'params' => [] ],
            'add_post'                => [ 'type' => 'create_post', 'params' => [] ],
            'write_post'              => [ 'type' => 'create_post', 'params' => [] ],

            // Page variations
            'new_page'                => [ 'type' => 'create_page', 'params' => [] ],
            'add_page'                => [ 'type' => 'create_page', 'params' => [] ],

            // Update variations
            'edit_post'               => [ 'type' => 'update_post', 'params' => [] ],
            'modify_post'             => [ 'type' => 'update_post', 'params' => [] ],
            'edit_page'               => [ 'type' => 'update_page', 'params' => [] ],
            'modify_page'             => [ 'type' => 'update_page', 'params' => [] ],

            // Delete variations
            'remove_post'             => [ 'type' => 'delete_post', 'params' => [] ],
            'trash_post'              => [ 'type' => 'delete_post', 'params' => [] ],
            'remove_page'             => [ 'type' => 'delete_post', 'params' => [] ],

            // File variations
            'create_file'             => [ 'type' => 'write_file', 'params' => [] ],
            'save_file'               => [ 'type' => 'write_file', 'params' => [] ],
            'remove_file'             => [ 'type' => 'delete_file', 'params' => [] ],

            // Plugin variations
            'install_plugin'          => [ 'type' => 'create_plugin', 'params' => [] ],
            'new_plugin'              => [ 'type' => 'create_plugin', 'params' => [] ],
            'enable_plugin'           => [ 'type' => 'activate_plugin', 'params' => [] ],
            'disable_plugin'          => [ 'type' => 'deactivate_plugin', 'params' => [] ],

            // Database variations
            'sql_query'               => [ 'type' => 'db_query', 'params' => [] ],
            'run_query'               => [ 'type' => 'db_query', 'params' => [] ],
            'database_query'          => [ 'type' => 'db_query', 'params' => [] ],
            'insert_row'              => [ 'type' => 'db_insert', 'params' => [] ],
            'update_row'              => [ 'type' => 'db_update', 'params' => [] ],
            'delete_row'              => [ 'type' => 'db_delete', 'params' => [] ],

            // WooCommerce variations (map to create_post with product type)
            'create_product'          => [ 'type' => 'create_post', 'params' => [ 'post_type' => 'product' ] ],
            'add_product'             => [ 'type' => 'create_post', 'params' => [ 'post_type' => 'product' ] ],
            'new_product'             => [ 'type' => 'create_post', 'params' => [ 'post_type' => 'product' ] ],
        ];

        // Check if action type needs normalization
        $lower_type = strtolower( $action_type );

        if ( isset( $mappings[ $lower_type ] ) ) {
            $mapping = $mappings[ $lower_type ];
            // Merge mapping params with original params (original takes precedence)
            $merged_params = array_merge( $mapping['params'], $params );

            $this->tracker->add_step( 'action_normalized', [
    ...
    private function execute_operation( string $action_type, array $params ): array {
        ...
        // Normalize action type (handle AI-generated variations)
        $normalized = $this->normalize_action_type( $action_type, $params );
